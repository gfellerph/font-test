...
const LevelOneAction = (props) => {
  ...
  return (index.h(TagName, { class: {
    ...
    index.h("span", null, props.level.text),
    index.h("svg", { "aria-hidden": "true" },
      index.h("use", { href: "#pi-pointy-arrow-right" }))));
};
...
const PostMainNavigation = class {
  constructor(hostRef) {
    ...
    this.flyoutToggled = index.createEvent(this, "flyoutToggled", 7);
  ...
  addFlyoutAnimation(flyout) {
    ...
    if (!ui_service.userPrefersReducedMotion()) {
  ...
  setBodyScroll() {
    if (this.mobileMenuOpen) {
      bodyScrollLock_esm.disableBodyScroll(this.flyoutElement);
    }
    else {
      bodyScrollLock_esm.enableBodyScroll(this.flyoutElement);
    }
  }
  /**
  ...
  render() {
    ...
    if (((_a = store.state.localizedConfig) === null || _a === void 0 ? void 0 : _a.header) === undefined)
      return;
    const headerConfig = store.state.localizedConfig.header;
    return (index.h(index.Host, null, index.h(svgSprite_component.SvgSprite, null), index.h("nav", { id: "post-internet-header-main-navigation", class: { 'main-navigation': true, 'open': this.mobileMenuOpen }, ref: el => (this.flyoutElement = el) }, index.h("h1", { class: "visually-hidden" }, headerConfig.translations.navMainAriaLabel), index.h("ul", { class: "main-container container" }, headerConfig.navMain.map(levelOne => (index.h("li", { key: levelOne.text, onMouseLeave: () => this.handleMouseLeave(levelOne), onMouseEnter: () => this.handleMouseEnter(levelOne) }, index.h(LevelOneAction, { level: levelOne, isOpen: this.isActiveFlyout(levelOne.id), onTouchEnd: e => this.handleTouchEnd(e, levelOne), onKeyDown: e => this.handleKeyPress(e, levelOne), onClick: e => this.handleClick(e, levelOne) }), !levelOne.noFlyout ? (index.h("div", { id: levelOne.id, class: { flyout: true, open: this.isActiveFlyout(levelOne.id) } }, index.h("div", { class: "wide-container" }, index.h("div", { class: "flyout-nav" }, index.h("button", { class: "nav-link flyout-back-button", onClick: () => this.closeFlyout(levelOne.id) }, index.h(svgIcon_component.SvgIcon, { name: "pi-pointy-arrow-right", classNames: "mirrored" }), index.h("span", null, headerConfig.translations.backButtonText)), index.h("button", { class: "flyout-close-button", onClick: () => this.closeFlyout(levelOne.id) }, index.h("span", { class: "visually-hidden" }, headerConfig.translations.mobileNavToggleClose), index.h(svgIcon_component.SvgIcon, { name: "pi-close" }))), index.h("h2", { class: "flyout-title container" }, index.h("a", { href: levelOne.url, class: "nav-link" }, levelOne.text)), index.h("div", { class: "flyout-row container" }, levelOne.flyout.map(flyout => (index.h("div", { key: flyout.title, class: "flyout-column" }, flyout.title ? index.h("h3", null, flyout.title) : null, index.h("ul", { class: "flyout-linklist" }, flyout.linkList.map(link => (index.h("li", { key: link.url }, index.h("a", { class: {
        'flyout-link': true,
        'active': !!(link === null || link === void 0 ? void 0 : link.isActiveOverride),
      }, href: link.url, target: link.target }, link.title)))))))))))) : null)))), index.h("slot", null))));
  }
  get host() { return index.getElement(this); }
};
PostMainNavigation.style = postMainNavigationCss;

exports.post_main_navigation = PostMainNavigation;

//# sourceMappingURL=post-main-navigation.cjs.entry.js.map